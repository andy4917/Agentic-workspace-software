...
def validate_manifest_shape(
    ...
    validate_default_prompt(interface, "defaultPrompt", errors)
    validate_default_prompt(interface, "default_prompt", errors)
...
def validate_default_prompt(
    ...
    if not isinstance(value, list) or len(value) == 0:
        errors.append(f"plugin.json field `interface.{key}` must be an array of strings")
        return
    if len(value) > 3:
        errors.append(f"plugin.json field `interface.{key}` must contain at most 3 prompts")
    for index, item in enumerate(value):
        if not isinstance(item, str) or not item.strip():
            errors.append(
                f"plugin.json field `interface.{key}[{index}]` must be a non-empty string"
            )
            continue
        if len(item) > 128:
            errors.append(
                f"plugin.json field `interface.{key}[{index}]` must be 128 characters or less"
            )
