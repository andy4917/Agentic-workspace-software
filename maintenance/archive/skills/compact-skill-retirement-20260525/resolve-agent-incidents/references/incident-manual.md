...
### Agent Role File Rejects Unknown Preferred Model Field

- Type: `codex_app_error`, `skill_or_doc_drift`
- Fingerprint: app startup or agent loading prints `Ignoring malformed agent role definition` with `unknown field preferred_model` for `agents\explorer.toml`.
- Risk: agents may think the configured explorer role is loaded while the app silently ignores the malformed role file.
- Likely cause: local role TOML schema accepts fields such as `name`, `description`, `developer_instructions`, `nickname_candidates`, and explicit `model` for roles that need it; `preferred_model` is not a supported role-file field.
- Fix playbook:
  1. Remove `preferred_model` from the live role TOML.
  2. Remove the same field from the harness role template so `apply` or regeneration does not reintroduce it.
  3. Keep model preference as prose in `developer_instructions` or use an app-supported `model` field only when the role must force a model.
  4. Update `.codex-harness/install-state.json` digest for the managed role file.
- Verification: `rg "preferred_model" agents maintenance/scripts` returns no matches; the agent role TOML parses; `codex_agent_harness.py doctor --json` reports `managed_files=pass`.
...
### Plugin Discovery Snapshot Versus Vendor Import Residue

- Type: `environment_path_issue`, `validation_gap`
- Fingerprint: `codex plugin list --marketplace openai-curated` shows `Path: ...\.tmp\plugins\.agents\plugins\marketplace.json`, `.tmp\plugins.sha` contains a 40-character commit hash, or `check-codex-native-alignment.ps1 -Json` fails on `vendor_imports`, incomplete `.tmp\plugins-clone-*`, or active config references to `.tmp\bundled-marketplaces`; `check-naming-conventions.ps1 -Json` may report same-name nested paths under a plugin snapshot unless that runtime snapshot is explicitly exempted.
- Risk: agents may keep accepting a regenerated broad curated marketplace snapshot as normal, leaving `.tmp\plugins` and `.tmp\plugins.sha` in place even when the workstation baseline says they are cleanup targets.
- Likely cause: current Codex CLI/plugin discovery can recreate an official `https://github.com/openai/plugins.git` snapshot in `.tmp\plugins` for curated marketplace listing; separate stale residue can still appear from failed clone/update attempts or vendor imports.
- Fix playbook:
  1. Preserve the failing command output and timestamps.
  2. If `.tmp\plugins` exists, validate that `.git\config` remote is `https://github.com/openai/plugins.git`, `.agents\plugins\marketplace.json` parses with name `openai-curated`, and `.tmp\plugins.sha` is a 40-character hex hash only to identify the regeneration source.
  3. Remove `.tmp\plugins` and `.tmp\plugins.sha` after preserving evidence; do not use them as active config sources and do not treat their internal `plugins\plugins` layout as acceptable workstation state.
  4. Add script-level guards so plugin and skill helpers refuse `CODEX_HOME\vendor_imports`, duplicate active roots, and active plugin-cache source destinations; keep incomplete `.tmp\plugins-clone-*` directories as cleanup targets.
  5. Move true residue to the Windows Recycle Bin after resolving each target and confirming it is under `%USERPROFILE%\.codex`; do not treat app-regenerated `.tmp\bundled-marketplaces` as a removable blocker, but remove regenerated `.tmp\plugins` and `.tmp\plugins.sha`.
  6. Rerun native alignment, naming convention, plugin listing, and harness doctor checks.
- Verification: after cleanup, `codex doctor --json`, `check-codex-native-alignment.ps1 -Json`, `check-naming-conventions.ps1 -Json`, and `codex_agent_harness.py doctor --json` all pass without `.tmp\plugins` or `.tmp\plugins.sha` present.
- Do not claim: a permanent upstream fix for plugin-list regeneration unless the discovery mechanism has been changed or disabled.
