...
- IAB recovery is not live-verified on the current Store app version
  `26.519.3891.0`.
...
1. Runtime-check the current Chrome backend against the active
   ...
   - arbitrary `chrome-extension://aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa/...` remains
     blocked.
2. Runtime-check the current IAB backend on Store app `26.519.3891.0`.
3. If IAB still fails with `No active Codex browser pane available`, inspect the
...
4. Apply any IAB app patch only after Codex Desktop exits, then restart and
...
- The active cache hotfix is static-verified but still needs fresh Chrome runtime
  proof on cache version `26.519.31651`.
- IAB remains unresolved until current-version runtime proof or current-version
  repair is complete.
